...
def build_similarity_graph_html(
    ...
    for i, row in graph_df.iterrows():
        cluster_id = int(row["cluster"])
        artist = _safe_text(row.get("artist", "unknown"))
        style = _safe_text(row.get("style", "unknown"))
        genre = _safe_text(row.get("genre", "unknown"))
        filename = os.path.basename(str(row["filepath"]))
        image_url = f"thumbs/{thumb_paths[i].name}"

        title_html = (
            f"<img src='{image_url}' width='160'><br>"
            f"<b>Artist:</b> {artist}<br>"
